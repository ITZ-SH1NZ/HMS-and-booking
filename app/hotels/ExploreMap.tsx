...
export default function ExploreMap({
  ...
  // Update Markers when hotels list or selection changes
  useEffect(() => {
    if (!map) return;
    ...
    if (hotels.some(h => h.latitude && h.longitude) && !selectedHotelId) {
      map.fitBounds(bounds, { padding: [40, 40] });
    }
  }, [map, hotels, selectedHotelId]);

  // Center map on selected hotel marker
  useEffect(() => {
  ...
  return (
    <div className="relative h-full w-full bg-slate-50 overflow-hidden">
      ...
      {selectedHotel && (
        <div className="absolute bottom-4 left-4 right-16 md:right-auto md:w-80 z-10 transition-all duration-300 animate-slide-up">
          <div className="relative flex gap-3 rounded-2xl bg-white p-3 shadow-2xl border border-slate-100 overflow-hidden">
            {/* Close Overlay Button */}
            <button
              onClick={() => onSelectHotel(null)}
              className="absolute top-1.5 right-1.5 flex h-5 w-5 items-center justify-center rounded-full bg-slate-100 text-slate-500 hover:bg-slate-200"
            >
